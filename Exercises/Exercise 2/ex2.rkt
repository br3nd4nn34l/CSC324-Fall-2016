...
(define (cartesian-product table1 table2)
  (apply append
         (map (λ(start)
                (all-concats start table2))
                table1)
  )
)

#|
(all-concats lst table)
  lst: list of objects [O1, O2, ...]
  table: list of lists [L1, L2, ..., Ln]

  Returns a list of the contatenation of lst and every list in table, in the 
  order [[O1, O2, ...] + L1, [O1, O2, ...] + L2, ...]

  If table is empty, return the empty list.

> (all-concats '(1 4) '((3 4 5) (2)))
'((1 4 3 4 5) (1 4 2))
|#
(define (all-concats lst table)
  (map (λ(end)
         (append lst end))
       table  
  )
...
(define (function-sort functions arg)
  (sort functions
        <
        ; We will sort based on a temporary function that outputs
        ;the value of fxn(arg), given function fxn
        #:key (λ(fxn)(fxn arg))
  )
)
